...
class RelationsTest extends \Tester\TestCase // @codingStandardsIgnoreLine
{
	...
	public function testObjectModify(Container $container): void
	{
		...
		Assert::notContains('electronic-technology', $stock->tags->toArray('uuid'));
		...
		Assert::count(0, $stock->tags->toArray('uuid'));
		
		
		// n:1
		$stock = $stocks->one('MS');
		$stock->alerts->relate(['test3']);
		Assert::contains('test3', $stock->alerts->toArray('uuid'));
		
		
		$stock2 = $stocks->one('IBM');
		$stock2->alerts->relate(['test3']);
		$stock->alerts->clear();
		Assert::notContains('test3', $stock->alerts->toArray('uuid'));
